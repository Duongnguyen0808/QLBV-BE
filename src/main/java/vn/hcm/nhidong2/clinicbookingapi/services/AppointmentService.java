...
public class AppointmentService {
 ...
 Appointment appointment = Appointment.builder()
  ...
  .status(AppointmentStatus.PAID_PENDING) // ĐẶT TRẠNG THÁI LÀ CHỜ THANH TOÁN
  .duration(duration)
 ...
 // KIỂM TRA QUY TẮC 6 GIỜ TRƯỚC KHI CHO PHÉP ĐỔI LỊCH
 ...
 if (hoursUntilAppointment < MINIMUM_CANCELLATION_LEAD_TIME_HOURS) {
  throw new IllegalStateException("Không thể đổi lịch hẹn trong vòng " + MINIMUM_CANCELLATION_LEAD_TIME_HOURS + " giờ trước thời điểm khám.");
 }
 ...
 // SỬA LOGIC QUAN TRỌNG: Nếu đã CONFIRMED (Đã thanh toán), KHÔNG CẦN thanh toán lại
 if (oldStatus == AppointmentStatus.CONFIRMED || oldStatus == AppointmentStatus.COMPLETED) {
appointment.setStatus(AppointmentStatus.CONFIRMED);
 } else {
appointment.setStatus(AppointmentStatus.PAID_PENDING); // ĐƯA VỀ CHỜ THANH TOÁN
 }
